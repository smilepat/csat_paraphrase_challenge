      ...
      <div className="mt-[0.8em] flex flex-wrap items-center gap-[0.6em]">
        <nav className="flex min-w-[18em] flex-1 gap-[0.5em]">
          {tasks.map((t, i) => (
            ...
            >
              <span className="block text-[0.8em] font-semibold opacity-80">유형 {t.view.type}</span>
              <span className="block text-[1.05em] font-bold">{AXIS_NAME[t.view.type]}</span>
            </button>
          ))}
        </nav>

        <div className="flex items-center gap-[0.4em]" aria-label="글자 크기">
          <span className="text-[0.75em] font-semibold text-[var(--color-muted)]">글자</span>
          {SIZES.map((s) => (
            <button
              key={s.key}
          ...
          {isSpanTask ? (
            <div className="space-y-[0.5em]">
              <p className="text-[0.9em] text-[var(--color-muted)]">
                왼쪽 지문에서 <b className="text-[var(--color-ink)]">되받는 대상</b>을 끌어서 표시한 뒤
                제출하세요.
              </p>
            ...
            <div className="space-y-[0.7em]">
              ...
              <p className="text-[0.75em] font-bold tracking-wide text-[var(--color-muted)]">답안 틀</p>
            ...
            <div className="space-y-[0.7em]">
              ...
              >
                <div className="flex items-baseline gap-[0.4em]">
                  <span data-testid="demo-score" className="text-[2.4em] font-bold leading-none tabular-nums">
                    {draft.result.score}
                  </span>
                ...
                <p className="mt-[0.5em] text-[1em] leading-relaxed">{draft.result.message}</p>
                ...
                {draft.result.model && (
                  <div className="mt-[0.7em] rounded-lg bg-white p-[0.6em]">
                    <p className="text-[0.8em] font-bold text-[var(--color-brand)]">모범답안</p>
                    <p className="mt-[0.2em] font-serif text-[1.05em]">{draft.result.model}</p>
